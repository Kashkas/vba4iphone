...
void LoadJoypadPic();
void DealBtnPress(SDL_Event *event, bool press = true);
...
*/
  ...
  systemSaveUpdateCounter = SYSTEM_SAVE_NOT_UPDATED;
	...
	//char szFile[] = "/Users/123acgf/Downloads/VBA/VisualBoyAdvance-1.7.2/rom.gbc";
	char szFile[] = "rom3.gba";
  if(/*optind < argc*/true) {
	...
	extern const char* SDL_GetDocumentPath();
	const char* strTemp = SDL_GetDocumentPath();
	printf("%s",strTemp);
	strcpy(saveDir, strTemp);
	strcpy(batteryDir, strTemp);
...
//  destHeight = (sizeOption+1)*srcHeight;
	...
	//destWidth = srcWidth = 320;
	//destHeight = srcHeight = 480;
  
//  surface = SDL_SetVideoMode(destWidth, destHeight, 16,
//                             SDL_ANYFORMAT|SDL_HWSURFACE|SDL_DOUBLEBUF|
//                             (fullscreen ? SDL_FULLSCREEN : 0));
	sdl_window = SDL_CreateWindow(NULL, 0, 0, destWidth, destHeight,
...
void systemDrawScreen()
{
  ...
  if(screenMessage) {
    ...
    if(((systemGetClock() - screenMessageTime) < 3000) &&
      ...
      drawText(pix, srcPitch, destx, desty*2,
               screenMessageBuffer); 
    } else {
      screenMessage = false;
    }
  ...
  if(filterFunction) {
    ...
    u8 *src = pix;
    u8 *dest = (u8*)surface->pixels + (destPitch*desty+destx*4);
    int i;
...
u32 systemReadJoypad(int which)
{
  ...
  if(sdlButtons[which][KEY_BUTTON_A]) {
	//sdlButtons[which][KEY_BUTTON_A] = false;
	printf("PRESS A BUTTION\n");
    res |= 1;
  }
	if(sdlButtons[which][KEY_BUTTON_B]) {
		printf("PRESS B BUTTION\n");
	//sdlButtons[which][KEY_BUTTON_B] = false;
    res |= 2;
	}
	if(sdlButtons[which][KEY_BUTTON_SELECT]){
		printf("PRESS SELECT BUTTION\n");
	//	sdlButtons[which][KEY_BUTTON_SELECT] = false;
	}
    res |= 4;
  if(sdlButtons[which][KEY_BUTTON_START]) {
	//sdlButtons[which][KEY_BUTTON_START] = false;
	printf("PRESS START BUTTION\n");
    res |= 8;
  }
	if(sdlButtons[which][KEY_RIGHT]) {
		printf("PRESS RIGHT BUTTION\n");
	//	sdlButtons[which][KEY_RIGHT] = false;
		res |= 16;
	}
    
	if(sdlButtons[which][KEY_LEFT]) {
		printf("PRESS LEFT BUTTION\n");
	//	sdlButtons[which][KEY_LEFT] = false;
		res |= 32;
	}
    
	if(sdlButtons[which][KEY_UP]) {
		printf("PRESS UP BUTTION\n");
	//	sdlButtons[which][KEY_UP] =false;
    res |= 64;
	}
	if(sdlButtons[which][KEY_DOWN]) {
		printf("PRESS DOWN BUTTION\n");
	//	sdlButtons[which][KEY_DOWN] = false;
    res |= 128;
	}
	if(sdlButtons[which][KEY_BUTTON_R]) {
		printf("PRESS R BUTTION\n");
	//	sdlButtons[which][KEY_BUTTON_R] = false;
    res |= 256;
	}
	if(sdlButtons[which][KEY_BUTTON_L]) {
		printf("PRESS L BUTTION\n");
	//	sdlButtons[which][KEY_BUTTON_R] = false;
    res |= 512;
	}
...
{
	//load joypad pic
	SDL_Surface *bmp_surface, *bmp_surface_rgba8888;
    ...
    bmp_surface = SDL_LoadBMP("vba_joypad.bmp");
	...
	u8 *scr = (u8*)bmp_surface_rgba8888->pixels;
	int scrPitch = bmp_surface_rgba8888->pitch;
	
	int destPitch = surface->pitch;
	u8 *dest = (u8*)surface->pixels + (destPitch*desty+destx*4);
	
	
	for (int i=0; i<bmp_surface_rgba8888->h; ++i) {
		memcpy(dest, scr, scrPitch);
		dest += destPitch;
		scr += scrPitch;
	}
...
#define IS_POINT_IN_RECT(x,y,rect) ((x)>=rect.x && (x)<=rect.x+rect.w && (y)>=rect.y && (y)<=rect.y+rect.h)
...
SDL_Rect ctrlbtn_positions[14] = 
{
	(SDL_Rect){0, 90, 40, 40},(SDL_Rect){81, 90, 40, 40},
	(SDL_Rect){41, 53, 39, 43},(SDL_Rect){41, 125, 39, 43},
	(SDL_Rect){241, 42, 79,83},(SDL_Rect){155, 114, 79, 83},
	(SDL_Rect){0, 190, 40, 40},(SDL_Rect){280, 190, 40, 40},
	(SDL_Rect){0, 0, 81, 28},(SDL_Rect){238, 0, 81, 28}
};
//int pressedBtn = -1;
...
std::map<SDL_FingerID, int> g_map;
...
void DealBtnPress(SDL_Event *event, bool press/*=true*/)
{
	...
	if (!press) {
		std::map<SDL_FingerID, int>::iterator it;
		it = g_map.find(fingerId);
		if (it != g_map.end()) {
			sdlButtons[0][g_map[fingerId]] = press;
			g_map.erase(it);
		}
		return;
	}
	...
	for (int i=0; i<10; ++i) {
		if (IS_POINT_IN_RECT(x, y, ctrlbtn_positions[i])) {			
			sdlButtons[0][i] = press;
			g_map[fingerId] = i;
			//pressedBtn = i;
			break;
		}
